...
@Service
public class CategoriesServiceImpl extends BaseServiceImpl<ProductCategories, CategoriesDTO>
    ...
    CategoriesServiceImpl(ProductCategoryUseCase productCategoryUseCase, ProductCategoryMapper categoryMapper,
    ...
    public ApiResponse<List<CategoriesDTO>> getAllByStoreId(String storeId) {
        List<CategoriesDTO> categoriesDTOS = productCategoryUseCase.findAllByStoreId(storeId).stream()
    ...
    public ApiResponse<CategoriesDTO> create(CategoriesDTO dto) {
        ...
        Store storeEntity = storeUseCase.findById(dto.getStoreId())
                .orElseThrow(() -> new NotFoundException("Store", dto.getStoreId()));
        p.setStore(storeEntity);
        return new ApiResponse<>(categoryMapper.toDTO(productCategoryUseCase.save(p)));
    }

    @Override
    public ApiResponse<List<CategoryWithProductDTO>> getAllCategoryWIthProduct() {
